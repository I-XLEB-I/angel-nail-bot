# Legacy: деплой на собственный Docker-сервер

Production сейчас разворачивается из GitHub в Railway. Актуальная инструкция:
[docs/railway_deploy.md](railway_deploy.md).

Этот документ оставлен только для случая осознанного возврата на отдельный VPS.
