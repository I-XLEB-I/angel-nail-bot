...
DEFAULT_STUDIO_ADDRESS_COPY_TEXT = "Очаковское шоссе, 5к4, подъезд 2"
LEGACY_STUDIO_ADDRESS_COPY_TEXT = "Очаковское шоссе, 5к3, подъезд 2"
...
async def load_studio_address_copy_text(repository: SettingRepository) -> str:
    ...
    value = await repository.get_value_or_default(
        STUDIO_ADDRESS_COPY_SETTING_KEY,
        DEFAULT_STUDIO_ADDRESS_COPY_TEXT,
    )
    return value.replace(
        LEGACY_STUDIO_ADDRESS_COPY_TEXT,
        DEFAULT_STUDIO_ADDRESS_COPY_TEXT,
    )
